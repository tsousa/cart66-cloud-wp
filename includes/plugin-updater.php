...
if ( ! defined( 'ABSPATH' ) || class_exists( 'Cart66_Plugin_Updater' ) )
	return;
...
class Cart66_Plugin_Updater{
	...
	public function plugins_api_result( $res, $action, $args ) {
		...
		if ( $plugin_slug == $updater_data['slug'] && $action == 'plugin_information' ){
			...
			if ( is_wp_error( $request ) ) {
				$res = new WP_Error( 'plugins_api_failed', '<p>' . __( 'An Unexpected HTTP Error occurred during the API request.', 'cart66-members' ) . '</p><p><a href="?" onclick="document.location.reload(); return false;">' . __( 'Try again', 'cart66-members' ) . '</a></p>', $request->get_error_message() );
			}
			...
			/* If no error, construct the data */
				...
				/* If data is empty or not an object */
					...
					$res = new WP_Error( 'plugins_api_failed', __( 'An unknown error occurred', 'cart66-members' ), wp_remote_retrieve_body( $request ) );
	...
	public function upgrader_post_install( $true, $hook_extra, $result ) {
		...
		if ( isset( $hook_extra ) ){
			...
			if ( isset( $hook_extra['plugin'] ) && $hook_extra['plugin'] == $plugin_base ){
				...
				/* Update message */
				$fail = __( 'The plugin has been updated, but could not be reactivated. Please reactivate it manually.', 'cart66-members' );
				$success = __( 'Plugin reactivated successfully. ', 'cart66-members' );
	...
	public function add_dashboard_widget() {
		...
		/* Widget name */
		$widget_name = $updater_data['name'] . __( ' Plugin Updates', 'cart66-members' );
	...
	public function dashboard_widget_callback() {
		...
		if ( !empty( $widget_option ) && is_array( $widget_option ) ){
			...
			if ( true === $updater_data['role'] ){
				...
				echo '<p>'. __( 'Username: ', 'cart66-members' ) . '<code>' . $username . '</code></p>';

				/* activation key input */
				$key = isset( $widget_option['key'] ) ? $widget_option['key'] : '' ;
				echo '<p>'. __( 'Email: ', 'cart66-members' ) . '<code>' . $key . '</code></p>';
			}
			else{

				/* activation key input */
				$key = isset( $widget_option['key'] ) ? $widget_option['key'] : '' ;
				echo '<p>'. __( 'Key: ', 'cart66-members' ) . '<code>' . $key . '</code></p>';
			}


			/* if key status is valid */
			if ( $widget_option['status'] == 'valid' ){
				_e( '<p>Your plugin update is <span style="color:green">active</span></p>', 'cart66-members' );
			}
			/* if key is not valid */
			elseif( $widget_option['status'] == 'invalid' ){
				_e( '<p>Your input is <span style="color:red">not valid</span>, automatic updates is <span style="color:red">not active</span>.</p>', 'cart66-members' );
				echo '<p><a href="' . $edit_url . '" class="button-primary">' . __( 'Edit Key', 'cart66-members' ) . '</a></p>';
			}
			/* else */
			else{
				_e( '<p>Unable to validate update activation.</p>', 'cart66-members' );
				echo '<p><a href="' . $edit_url . '" class="button-primary">' . __( 'Try again', 'cart66-members' ) . '</a></p>';
			}
		}
		/* if activation key is not yet set/empty */
		else{
			echo '<p><a href="' . $edit_url . '" class="button-primary">' . __( 'Add Key', 'cart66-members' ) . '</a></p>';
		}
	}
	...
	public function dashboard_widget_control_callback() {
		...
		<p>
			<label for="<?php echo $widget_id; ?>-username"><?php _e( 'User name', 'cart66-members' ); ?></label>
		</p>
		<p>
			<input id="<?php echo $widget_id; ?>-username" name="<?php echo $widget_id; ?>[username]" type="text" value="<?php echo $username_option;?>"/>
		</p>
		<p>
			<label for="<?php echo $widget_id; ?>-key"><?php _e( 'Email', 'cart66-members' ); ?></label>
		</p>
		<p>
		...
		<p>
			<label for="<?php echo $widget_id; ?>-key"><?php _e( 'Activation Key', 'cart66-members' ); ?></label>
		</p>
		<p>
			<input id="<?php echo $widget_id; ?>-key" class="regular-text" name="<?php echo $widget_id; ?>[key]" type="text" value="<?php echo $key_option;?>"/>
		</p>
